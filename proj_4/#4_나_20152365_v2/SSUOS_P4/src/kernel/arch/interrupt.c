...
// tick 계산 함수 // 현재 프로세스의 ticks 계산
void timer_handler(struct intr_frame *iframe)
{
	ticks++;

	// 스케줄링 중이 아니고, idle process가 아니라면 (proc1, proc2, proc3 tick 계산)
	if (scheduling != 1 && cur_process->pid != 0) { 
		cur_process->time_slice++;	
		cur_process->time_used++;
	} // idle process tick 계산
	if (cur_process->pid == 0) {
		cur_process->time_slice++;
		cur_process->time_used++;
	}

	// time_slice를 모두 소비한 경우 schedule 함수 실행
	if (cur_process->time_slice >= TIMER_MAX && cur_process->pid != 0) {
		do_sched_on_return();
	}

	// 30틱마다 우선순위 재계산
	if ((cur_process->time_slice)%30 == 0 && cur_process->pid != 0)
